...
type CustomControllerProps = {
    name: string
    type: 'text' | 'checkbox' | 'checkbox' | 'password' | 'select' | 'email'
    control: any
    label?: string
    required?: boolean
    maxLength?: number
...
export const CustomController: React.FC<CustomControllerProps> = ({control, type, name, label, required, maxLength, minLength, pattern, onBlur, styleProps, options}) => {
    let rules = {}
    if (required) {
        rules = {
            ...rules,
    ...
    return(
        <Controller
            ...
            render={({ field, fieldState }) => (
                ...
                type==='text' ? (<Form.Item label={label} required={required} validateStatus={fieldState.invalid ? 'warning' : 'success'} help={fieldState.invalid ? fieldState.error?.message : null}> <Input {...field} /></Form.Item>) :
