...
export const SysTasks: React.FC<SysTasks> = ({ mySysDataIdArray, mySysData, rec, error, access, tasksChanges }) => {
    ...
    const protoData = [
        {
            ...
            value: <ProtocolUpload changes={tasksChanges?.find(e => e.taskChangeType === 'vp')} data={data[0]} rec={rec} mySysDataIdArray={mySysDataIdArray} error={error} objectType='systems' access={access} />
        },
        {
            rowName: <Text style={widthScreen < 1370 ? { fontSize: '9pt' } : widthScreen < 1605 ? {} : { fontSize: '12pt' }}>Код протокола</Text>,
            value: <ProtocolCode changes={tasksChanges?.find(e => e.taskChangeType === 'nvp')} data={data[0]} rec={rec} mySysDataIdArray={mySysDataIdArray} objectType='systems' access={access} />
        },
        {
            rowName: <Text style={widthScreen < 1370 ? { fontSize: '9pt' } : widthScreen < 1605 ? {} : { fontSize: '12pt' }}>Дата утверждения протокола</Text>,
            value: <DatePickerForWork changes={tasksChanges?.find(e => e.taskChangeType === 'dvp')} date={data[0].dvp} objectId={data[0].idfromtable} dateType='dvp' id={data[0].id} key={data[0].id} group={rec.objectType} myDataIdArray={mySysDataIdArray} access={access} />
        },
    ]
    ...
    const reportData = [
        {
            ...
            value: <ReportUpload changes={tasksChanges?.find(e => e.taskChangeType === 'vo')} data={data[0]} rec={rec} mySysDataIdArray={mySysDataIdArray} error={error} objectType='systems' access={access} />
        },
        {
            rowName: <Text style={widthScreen < 1370 ? { fontSize: '9pt' } : widthScreen < 1605 ? {} : { fontSize: '12pt' }}>Код отчета</Text>,
            value: <ReportCode changes={tasksChanges?.find(e => e.taskChangeType === 'nvo')} data={data[0]} rec={rec} mySysDataIdArray={mySysDataIdArray} objectType='systems' access={access} />
        },
        {
            rowName: <Text style={widthScreen < 1370 ? { fontSize: '9pt' } : widthScreen < 1605 ? {} : { fontSize: '12pt' }}>Дата утверждения отчета</Text>,
            value: <DatePickerForWork changes={tasksChanges?.find(e => e.taskChangeType === 'dvo')} date={data[0].dvo} objectId={data[0].idfromtable} dateType='dvo' id={data[0].id} key={data[0].id} group={rec.objectType} myDataIdArray={mySysDataIdArray} access={access} />
        },
    ]
    ...
    const labelData = [
        {
            ...
            value: <LabelStatus changes={tasksChanges?.find(e => e.taskChangeType === 'et')} data={data[0]} mySysDataIdArray={mySysDataIdArray} objectType='systems' access={access} />
        },
    ]
            ...
            value: <>
                ...
                <Modal afterOpenChange={() => handleCancel('TeachProtocol')} title="Протокол обучения персонала" open={TeachProtocolModalOpen} onCancel={() => handleCancel('TeachProtocol')} footer={[<Button key="close" onClick={() => handleCancel('TeachProtocol')} type="primary">Закрыть</Button>]} >
                    <iframe
                        ...
                        src={`${server}API/PrintForms/add_b.php?id=${data[0].id}&idfromtable=${data[0].idfromtable}&tp=sys.work&user&stroki=5&typeForm=2`}></iframe>
                </Modal>
            </>
        },
        {
    ...
    return <>
        <Row style={{ margin: '20px 0' }}>
            <Col span={3} push={1} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: 'center' }}>
                <ProgressStatus record={rec} mySysData={mySysData} />
            </Col>
            <Col span={9} push={2}>
                <Table
                    columns={columns}
